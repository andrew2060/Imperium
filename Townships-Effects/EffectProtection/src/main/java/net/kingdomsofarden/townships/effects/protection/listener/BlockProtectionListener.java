...
public class BlockProtectionListener implements Listener {
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        if (event.getAction().equals(Action.RIGHT_CLICK_BLOCK) || event.getAction()
            ...
            if (Constants.ACCESS_TYPES.contains(b.getType())) {
                TreeSet<Region> boundingRegions =
                    plugin.getRegions().getBoundingRegions(event.getClickedBlock().getLocation());
                ...
                if (!hasAccess(boundingRegions, c, AccessType.ACCESS)) {
                    ...
                    Messaging
                        .sendFormattedMessage(event.getPlayer(), I18N.NO_PERMISSION_AREA_ACCESS);
                }
            } else if (Constants.INTERACT_TYPES.contains(b.getType()) || (Constants.PROTECT_FIRE
                && event.getItem() != null && event.getItem().getType()
                ...
                if (!hasAccess(boundingRegions, c, AccessType.INTERACT)) {
                    event.setCancelled(true);
                    Messaging
                        .sendFormattedMessage(event.getPlayer(), I18N.NO_PERMISSION_AREA_INTERACT);
                }
            }
        }
    }
    ...
    public void onBlockPlace(BlockPlaceEvent event) {
        TreeSet<Region> boundingRegions =
            plugin.getRegions().getBoundingRegions(event.getBlock().getLocation());
        Citizen c = Townships.getCitizens().getCitizen(event.getPlayer().getUniqueId());
